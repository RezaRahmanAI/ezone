...
namespace IMSWEB.Controllers
{
    ...
    public class DamagePurchaseOrderController : CoreController
    {
        ...
        public DamagePurchaseOrderController(IErrorService errorService,
            ...
            IGodownService godownService, IUserAuditDetailService userAuditDetailService,
            IProductService productService, ISisterConcernService SisService, IExcelReaderService excelReaderService, ISessionMasterService sessionMasterService, IRoleService roleService, ISystemInformationService sysInfoService, IDOService dOService
            )
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var customPO = _purchaseOrderService.GetAllDamagePurchaseOrderAsync(DateRange.Item1, DateRange.Item2, IsVATManager(), User.Identity.GetConcernId());
        ...
        public async Task<ActionResult> Index(FormCollection formCollection)
        {
            ...
            var customPO = _purchaseOrderService.GetAllDamagePurchaseOrderAsync(ViewBag.FromDate, ViewBag.ToDate, IsVATManager(), User.Identity.GetConcernId());
            var vmPO = _mapper.Map<IEnumerable<Tuple<int, string, DateTime, string, string, string, EnumPurchaseType, Tuple<int>>>,
                IEnumerable<GetPurchaseOrderViewModel>>(await customPO);
        ...
        {
            //bool Result = _purchaseOrderService.IsPOReturn(orderId);
            //if (Result == true)
            //{
            ...
            //if (orderId > 0)
            //{
            //    var purchase = _purchaseOrderService.GetPurchaseOrderById(orderId);
            //    var purchasedetails = _purchaseOrderDetailService.GetPOrderDetailsByPurchaseOrderId(purchase.POrderID);
            //    if (purchasedetails.POProductDetails.Any())
            //    {
            //        if (purchasedetails.POProductDetails.Sum(d => d.ReturnQty) > 0)
            //        {
            ...
            //if (HasReturnProductCheckByPOId(orderId))
            //{
            //    AddToastMessage("", "Some product(s) has already been return from this order. Can't edit this purchase.",
            //        ToastType.Error);
            //    return RedirectToAction("Index");
            //}
            ViewBag.IsDOShow = _sysInfoService.IsDOShow();

            if (_purchaseOrderService.IsReturnFound(orderId))
            {
                AddToastMessage("", "Some product(s) has already been returned from this order. Can't edit this Purchase.",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
            ...
            if (HasHireSalesProductCheckByPOId(orderId))
            {
                AddToastMessage("", "Some product(s) has already been sold from this order. Can't edit this purchase.",
                    ToastType.Error);
                return RedirectToAction("Index");
            ...
            {
                AddToastMessage("", "Some product(s) has already been transfer from this order. Can't edit this purchase.",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
        ...
        public ActionResult Delete(int orderId)
        {
            if (_purchaseOrderService.IsReturnFound(orderId))
            {
                AddToastMessage("", "Some product(s) has already been returned from this order. Can't return this Purchase.",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
            ...
            if (HasSoldProductCheckByPOId(orderId))
            {
                AddToastMessage("", "Some product(s) has already been sold from this order. Can't delete this Purchase",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
            if (HasHireSalesProductCheckByPOId(orderId))
            ...
            if (HasTransferProductCheckByPOId(orderId))
            {
                AddToastMessage("", "Some product(s) has already been transfer from this order. Can't delete this Purchase",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
            ...
            var Sodetailss = _purchaseOrderDetailService.GetPODetailByPOPDID(orderId);

            if (Sodetailss.DOrderDetailId > 0)
            {
                var DODetails = _dOService.GetDetailById(Convert.ToInt32(Sodetailss.DOrderDetailId));
                if (DODetails != null)
                {
                    DODetails.GivenQty -= Sodetailss.Quantity;
                    _dOService.UpdateDoDetails(DODetails);
                    _dOService.Save();
        ...
        public ActionResult EditFromView(int id, int cid, string previousAction, bool IsDO)
        {
            ...
            if (itemToEdit != null)
            {
                ...
                {
                    if (itemToEdit.DOrderDetailID > 0)
                    {
                        var doDetail = _dOService.GetAllDetail().FirstOrDefault(i => i.DODID == itemToEdit.DOrderDetailID);
                        if (doDetail != null)
                        {
                            var oDO = _dOService.GetById(doDetail.DOID);
                            itemToEdit.DOID = doDetail.DOID;
                            itemToEdit.DONo = oDO.DONo;
                        }
        ...
        void POrderUpdate(PurchaseOrderViewModel purchaseOrder, CreatePurchaseOrderDetailViewModel PorderDetail)
        {
            ...
            purchaseOrder.PurchaseOrder.NetDiscount = (decimal.Parse(GetDefaultIfNull(purchaseOrder.PurchaseOrder.NetDiscount))
                                                        - TotalPPDisAmt).ToString();
            purchaseOrder.PurchaseOrder.tempNetDiscount = (decimal.Parse(GetDefaultIfNull(purchaseOrder.PurchaseOrder.tempNetDiscount))
                                            - TotalPPDisAmt).ToString();
        ...
        private void CheckAndAddModelErrorForAdd(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel PurchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            else
            {
                ...
                if (newPurchaseOrder.PODetail.DOID > 0)
                {
                    int ProductID = Convert.ToInt32(PurchaseOrder.PODetail.ProductId);
                    int ColorID = string.IsNullOrEmpty(formCollection["ColorsId"]) ? 0 : Convert.ToInt32(formCollection["ColorsId"]);

                    var Detail = _dOService.GetDetailsById(newPurchaseOrder.PODetail.DOID)
                                    .FirstOrDefault(i => i.ProductID == ProductID);
                    decimal qty = Convert.ToDecimal(newPurchaseOrder.PODetail.Quantity);
                    if (Detail == null)
                        ModelState.AddModelError("PODetail.Quantity", "This product is not found in this DO");
                    else if ((Detail.DOQty - Detail.GivenQty) < qty)
                        ModelState.AddModelError("PODetail.Quantity", "DO qty(" + (Detail.DOQty - Detail.GivenQty) + ") is not available");
                }
            }
        ...
        private void AddToOrder(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel purchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            purchaseOrder.PODetail.DONo = newPurchaseOrder.PODetail.DONo;
            purchaseOrder.PODetail.DOID = newPurchaseOrder.PODetail.DOID;
        ...
        private void SaveOrder(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel purchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            if (IsDeliveryOrder)
                ...
                purchaseOrder.PurchaseOrder.Status = Convert.ToString((int)EnumPurchaseType.DamageOrder);
            ...
            else
            {
                ...
                {
                    Tuple<bool, int> dbResult = _purchaseOrderService.AddDamagePurchaseOrderUsingSP(dtPurchaseOrder, dtPurchaseOrderDetail, dtPOProductDetail, dtStock, dtStockDetail);
                    if (dbResult.Item1)
                    {
        ...
        private DataSet CreatePODetailDataTable(PurchaseOrderViewModel purchaseOrder, DataTable dtStock)
        {
            ...
            foreach (var item in purchaseOrder.PODetails)
            {
                ...
                if (item.DOID > 0)
                {
                    ...
                    var DoDetail = _dOService.GetDetailsById(item.DOID).FirstOrDefault(i => i.ProductID == ProductID);
                    if (DoDetail != null)
        ...
        private ActionResult ReturnCreateViewWithTempData(bool IsDeliveryOrder)
        {
            ...
            else
            {
                ...
                return View(new PurchaseOrderViewModel
                {
                    ...
                    PurchaseOrder = new CreatePurchaseOrderViewModel { ChallanNo = chllnNo, InvoiceNo = chllnNo }
                });
            }
        }
        ...
        private ActionResult HandlePurchaseOrder(PurchaseOrderViewModel newPurchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            if (newPurchaseOrder != null)
            {
                ...
                if (formCollection.Get("addButton") != null)
                {
                    ...
                    //ModelState.Remove("PODetail.SalesRate");
                    if (!ModelState.IsValid)
                    {
                        if (IsDeliveryOrder)
                            return View("CreateDeliveryOrder", purchaseOrder);
        ...
        private bool HasHireSalesProductCheckByPOId(int id)
        {
            int sold = _purchaseOrderService.CheckHireSalesProductStatusByPOId(id);
            return sold > 0;
        }
        ...
        [Authorize]
        public ActionResult InvoiceHistory(int orderId, bool IsDO)
        {
        ...
        [HttpGet]
        [Authorize]
        public ActionResult SupplierLedgerSummary()
        {
            return View();
        }
        ...
        public ActionResult EditPermission(int orderId)
        {
            if (orderId != 0)
            {
                var obj = _purchaseOrderService.GetPurchaseOrderById(orderId);
                obj.EditReqStatus = 1;
                _purchaseOrderService.Update(obj);
                _purchaseOrderService.SavePurchaseOrder();
