...
{
    [Authorize]
    [RoutePrefix("emobilepurchaseorder")]
    public class EmobilePurchaseOrderController : CoreController
    {
        ...
        public EmobilePurchaseOrderController(IErrorService errorService,
            ...
            IProductService productService, ISisterConcernService SisService, IExcelReaderService excelReaderService, ISessionMasterService sessionMasterService, IRoleService roleService, ISystemInformationService sysInfoService
            )
        ...
        public ActionResult Delete(int orderId)
        {
            if (_purchaseOrderService.IsReturnFound(orderId))
            {
                AddToastMessage("", "Some product(s) has already been returned from this order. Can't delete this Purchase.",
                    ToastType.Error);
                return RedirectToAction("Index");
            }
        ...
        private void AddToOrder(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel purchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            purchaseOrder.PurchaseOrder.VATPercentage = decimal.Parse(GetDefaultIfNull(newPurchaseOrder.PurchaseOrder.VATPercentage)).ToString();
            purchaseOrder.PurchaseOrder.VATAmount = decimal.Parse(GetDefaultIfNull(newPurchaseOrder.PurchaseOrder.VATAmount)).ToString();
        ...
        private void SaveOrder(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel purchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            purchaseOrder.PurchaseOrder.Remarks = newPurchaseOrder.PurchaseOrder.Remarks;
            purchaseOrder.PurchaseOrder.VATPercentage = newPurchaseOrder.PurchaseOrder.VATPercentage;
            purchaseOrder.PurchaseOrder.VATAmount = newPurchaseOrder.PurchaseOrder.VATAmount;
        ...
        private ActionResult HandlePurchaseOrder(PurchaseOrderViewModel newPurchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            if (newPurchaseOrder != null)
            {
                ...
                else if (formCollection.Get("submitButton") != null)
                {
                    ...
                    if ((Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.GrandTotal) != calGrandTotal) || (Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.GrandTotal) + Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.LabourCost)) != Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.TotalAmount) + Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.NetDiscount) + Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.AdjAmount) - Convert.ToDecimal(newPurchaseOrder.PurchaseOrder.VATAmount))
                    {
                        TempData["purchaseOrderViewModel"] = null;
                        ModelState.Clear();
                        AddToastMessage("", "Order has been failed. Please try again.", ToastType.Error);
