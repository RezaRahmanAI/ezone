...
{
    [Authorize]
    [RoutePrefix("Barcodecredit-sales-order")]
    public class BarcodeCreditSalesOrderController : CoreController
    {
        ...
        public BarcodeCreditSalesOrderController(IErrorService errorService,
            ...
            ISisterConcernService SisterConcernService, ISessionMasterService sessionMasterService, IRoleService roleService, IUserAuditDetailService userAuditDetailService, ISMSBillPaymentBkashService sMSBillPaymentBkashService)
        ...
        public async Task<ActionResult> Index(FormCollection formCollection)
        {
            ...
            var customSO = _creditSalesOrderService.GetAllSalesOrderAsync(ViewBag.FromDate, ViewBag.ToDate,
                IsVATManager(), User.Identity.GetConcernId(), InvoiceNo, ContactNo, CustomerName, AccountNo);
            var vmSO = _mapper.Map<IEnumerable<Tuple<int, string, DateTime, string, string, decimal, EnumSalesType, Tuple<string, int>>>,
                IEnumerable<GetCreditSalesOrderViewModel>>(await customSO);
            return View(vmSO);
        }
            ...
            if (newSalesOrder != null)
            {
                ...
                else if (formCollection.Get("submitButton") != null)
                {
                    ...
                    if (SystemInfo.IsHireSMSEnable == 1 && salesOrder.SalesOrder.IsSmsEnable == true)
                    {
                        ...
                        {
                            var response = await Task.Run(() => SMSHTTPService.SendSMSAsync(EnumOnnoRokomSMSType.NumberSms, sms, previousBalance, SystemInfo, User.Identity.GetUserId<int>()));
            ...
            CreditSalesOrderViewModel salesOrder, FormCollection formCollection)
        {
            ...
            if (oPaidSchedule != null)
            {
                ...
                if (SystemInfo.IsInstallmentSMSEnable == 1 && salesOrder.SalesOrder.IsSmsEnable == true)
                {
                    ...
                    if (smsAmountDetails.TotalRecAmt > 1)
                    {
                        var Response = SMSHTTPService.SendSMS(EnumOnnoRokomSMSType.OneToOne, sms, previousBalance, SystemInfo, User.Identity.GetUserId<int>());
                        if (Response.Count > 0)
                        {
            ...
            CreditSalesOrderViewModel salesOrder, FormCollection formCollection)
        {
            ...
            //{
            //    if (_creditSalesOrderService.AddPendingSalesOrderUsingSP(dtSalesOrder, dtSalesOrderDetail,
            //        dtSalesOrderSchedules))
            //    {
            //        IsSuccess = true;
            ...
            //else
            //{
            //    if (_creditSalesOrderService.AddSalesOrderUsingSP(dtSalesOrder, dtSalesOrderDetail, dtSalesOrderSchedules, dtBankTrans))
            //    {
        ...
        private DataTable CreateSalesOrderDataTable(CreditSalesOrderViewModel salesOrder)
        {
            ...
            row["CreatedDate"] = GetLocalDateTime().Date;
