...
namespace IMSWEB.Controllers
{
    ...
    public class SalesOrderController : CoreController
    {
            ...
            foreach (var SODetail in newSalesOrder.SODetails)
            {
                ...
                if (SODetail.ProductType == (int)EnumProductType.NoBarcode)
                {
                    ...
                    //StockQty = StockCount.Quantity;
                     ...
                     StockQty = _SRVisitService.GetRemainingQuantityForSRVisit(stockDeatilCount.ProductID, stockDeatilCount.ColorID, User.Identity.GetConcernId(), stockDeatilCount.GodownID, 0, stockDeatilCount.SDetailID);
                ...
                else
                {
                    ...
                    //    oDetail = _stockService.GetIMEIDetail(SODetail.IMENo);
                    //}
                    //else
                    //{
                    //    SODetail.Status = EnumStatus.New;
                    ...
                    //        AddToastMessage(message: $"IMEI {oDetail.IMENo} is not available in the stock", toastType: ToastType.Error);
                    //        return;
                    //    }
                    //}
        ...
        private bool SaveOrder(
            SalesOrderViewModel salesOrder, FormCollection formCollection, bool isPreview)
        {
            bool Result = false;
        ...
        {
            ViewBag.IsDateWiseProductPicker = _SysInfoService.IsDateWiseProductPicker();
            ViewBag.IsHoqueSOrderSalesRate = _SysInfoService.IsHoqueSOrderSalesRate();
            SalesOrderViewModel salesOrder = (SalesOrderViewModel)TempData.Peek("salesOrderViewModel");
            if (salesOrder != null)
            {
                TempData["salesOrderViewModel"] = salesOrder;
        ...
        private ActionResult HandleSalesOrder(SalesOrderViewModel newSalesOrder, FormCollection formCollection)
        {
            ...
            if (newSalesOrder != null)
            {
                ...
                //    return View("Create", salesOrder);
                //}
                if (formCollection.Get("submitButton") != null || formCollection.Get("btnPreview") != null)
                {
                    ...
                    bool Result = SaveOrder(newSalesOrder, formCollection, isPreview);
        ...
        [HttpGet]
        [Authorize]
        public ActionResult WarrantyInvoice(int orderId)
        {
            TempData["WarrantyInvoice"] = true;
            TempData["OrderId"] = orderId;
            return RedirectToAction("Index");
        }
